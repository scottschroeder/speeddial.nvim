local async = require("speeddial.async")
local lazy = require("speeddial.lazy")
local oop = require("speeddial.oop")

local Mock = lazy.access("speeddial.mock", "Mock") ---@type Mock|LazyModule
local Semaphore = lazy.access("speeddial.control", "Semaphore") ---@type Semaphore|LazyModule
local loop = lazy.require("speeddial.debounce") ---@module "speeddial.debounce"
...
function Logger:init(opt)
  opt = opt or {}
  self.plugin = opt.plugin or "speeddial"
  self.outfile = opt.outfile or fmt("%s/%s.log", vim.fn.stdpath("cache"), self.plugin)
  self.outfile_status = Logger.OutfileStatus.UNKNOWN
  self.level = SpeeddialGlobal.debug_level > 0 and Logger.LogLevels.debug or Logger.LogLevels.info
  self.msg_buffer = {}
...
function Logger:lvl(min_level)
  if SpeeddialGlobal.debug_level >= min_level then
    return self
  end
...
---@param job speeddial.Job
---@param opt? Logger.log_job.Opt
function Logger:log_job(job, opt)
  opt = opt or {}

  if opt.silent then return end
  if opt.debug_level and SpeeddialGlobal.debug_level < opt.debug_level then
    return
  end
