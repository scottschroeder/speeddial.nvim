...
describe("speeddial.lib.project", function()
  ...
  describe("normalize", function()
    it("relative to cwd", function()
      local p1 = Project({ vcs_root = "foo" })
      eq(p1:as_path(), vim.fn.getcwd() .. "/foo")
    end)
    ...
    it("normalized", function()
      local p1 = Project({ vcs_root = "/foo/bar/../baz" })
      eq(p1:as_path(), "/foo/baz")
    end)

    it("expand user home", function()
      local p1 = Project({ vcs_root = "~/foo" })
      local homedir = vim.loop.os_homedir()
      eq(p1:as_path(), homedir .. "/foo")
    end)
  end)
end)
