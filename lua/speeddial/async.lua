local ffi = require("speeddial.ffi")
...
local function dstring(object)
  if not SpeeddialGlobal.logger then return "" end
  dstring = SpeeddialGlobal.logger.dstring
...
---@class Waitable : speeddial.Object
...
function Future:dprint(...)
  if not SpeeddialGlobal.logger then return end
  if SpeeddialGlobal.debug_level >= 10 or M._watching[self] then
    local t = { self, "::", ... }
    for i = 1, table.maxn(t) do t[i] = dstring(t[i]) end
    SpeeddialGlobal.logger:debug(table.concat(t, " "))
  end
end
